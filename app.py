...
def analyze_stock(stock_info, strategy, selected_patterns):
    ticker, stock_name, volume_shares, f_consec, t_consec = stock_info
    try:
        ...
        if is_match:
            return {
                ...
                "今日成交量(股)": int(volume_shares), # 💡 改為股數
                "外資連動天數": int(f_consec), 
                "投信連動天數": int(t_consec), 
                "技術型態訊號": pattern_name
            }
...
def plot_kline(yf_ticker, stock_name):
    ...
    fig = make_subplots(
        ...
        row_heights=[0.55, 0.2, 0.25], subplot_titles=("", "成交量 (股)", "KD 指標 (9,3,3)") # 💡 標題改為股
    )
    ...
    stats = {'Close': latest['Close'], 'Change': change, 'ChangePct': (change / prev['Close']) * 100, 'Volume': int(latest['Volume'])} # 💡 不再除以 1000
    return fig, stats
...
# 💡 將預設門檻從 500張 改為 500,000股，並更新提示文字
min_volume = st.sidebar.number_input("📉 核心成交量門檻 (股數 >):", min_value=0, max_value=50000000, value=500000, step=100000)
        ...
        with st.spinner("正在執行漏斗式多維度交叉過濾 (📡 正在向 Yahoo 請求全市場即時成交量)..."):
            ...
            if not df_all.empty:
                ...
                try:
                    ...
                    # 💡 取消 / 1000 的換算，直接保留原始股數
                    df_filtered['Volume_Shares'] = df_filtered['YF_Ticker'].map(latest_vols)
                    df_filtered['Volume_Shares'] = df_filtered['Volume_Shares'].fillna(0).astype(int)
                ...
                # 💡 確保排序和篩選都使用新的股數變數
                if "Top 500" in hot_filter: df_filtered = df_filtered.nlargest(500, 'Volume_Shares')
                elif "Top 100" in hot_filter: df_filtered = df_filtered.nlargest(100, 'Volume_Shares')
                df_filtered = df_filtered[df_filtered['Volume_Shares'] >= min_volume]

                st.info(f"✅ 即時流動性與籌碼過濾完畢，共 **{len(df_filtered)}** 檔股票進入技術形態掃描...")

                active_pool = [
                    (row['YF_Ticker'], row['Name'], row['Volume_Shares'], row['Foreign_Days'], row['Trust_Days']) 
                ...
                if results:
                    df_final = pd.DataFrame(results).sort_values(by="今日成交量(股)", ascending=False)
            ...
            if selected_stock:
                ...
                with st.spinner("載入即時 K 線與圖表中..."):
                    ...
                    if fig and stats:
                        ...
                        # 💡 加入千分位符號 (:,)，讓百萬等級的股數更好閱讀
                        metric_cols[1].metric("今日成交股數", f"{stats['Volume']:,} 股")
